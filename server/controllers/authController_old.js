...
const register = async (req, res) => {
  try {
    ...
    // Check if user is under 13 - block them completely
    const isUnder13 = calculatedAge && calculatedAge < 13;

    // Block users under 13 completely
    if (isUnder13) {
      console.log('Blocking under-13 user registration');
      return res.status(400).json({
        success: false,
        message: 'Users under 13 are not allowed to create accounts. Please ask your parent to create an account for you.',
        isUnder13: true,
        blockedReason: 'Age restriction'
      });
    }
...
const login = async (req, res) => {
  try {
    ...
    // Check if user is under 13 - they should be blocked completely
    const isUnder13 = user.age && user.age < 13;
    if (isUnder13 || user.status === 'inactive') {
      console.log('Under-13 user or inactive status - account blocked:', email);
      
      return res.status(403).json({
        success: false,
        message: 'Users under 13 are not allowed. Please ask your parent to create an account for you.',
        isUnder13: true,
        isAccountBlocked: true,
        blockedReason: 'Age restriction - users under 13 not allowed'
      });
    }
...
const googleAuth = async (req, res) => {
  try {
    ...
    const isUnder13 = user.age && user.age < 13;
    if (isUnder13 || user.status === 'inactive') {
      console.log('Google OAuth: Under-13 user or inactive status - account blocked:', email);
...
// @desc    Convert to child role (for users 25+)
// @route   POST /api/auth/convert-to-child
// @access  Private
const convertToChildRole = async (req, res) => {
  try {
    console.log('Child role conversion request received:', {
      body: req.body,
      userId: req.userId
    });
    ...
    console.log('User converted to child role:', {
      email: user.email,
      age: user.age,
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to convert to child account',
      error: error.message
    });
  }
};
